...
public class Xgt_sub_task_objController extends BaseController {
	...
	//作业对象列表查询
    @RequestMapping(value = "/nowTasktList")
    @ResponseBody
    public Object nowTasktList(@RequestParam Integer Id) {
        List<Map<String, Object>>list=this.xgt_sub_task_objDao.nowTasktList(Id);
        return list;
    }

    @RequestMapping(value = "/IfWancheng")
    @ResponseBody
    public Integer IfWancheng(@RequestParam Integer task_id,@RequestParam Integer over_id){
        Integer wanchengId=this.xgt_sub_task_objDao.IfWancheng(task_id,over_id);
        return  wanchengId;
    }
}
